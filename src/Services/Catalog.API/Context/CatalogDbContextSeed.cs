...
namespace Catalog.API.Context
{
    public class CatalogDbContextSeed
    {
        ...
        {
            //check is there any product in the database
            var products = _productManager.GetFirstOrDefault(c => true);
            //if there is no product in the database, seed the database
            if (products == null)
            {
                _productManager.Add(GetPreconfigureProducts());
            }
        }

        private static List<Product> GetPreconfigureProducts()
        {
